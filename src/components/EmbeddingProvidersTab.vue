...
/**
 * EmbeddingProvidersTab —— 向量嵌入供应商管理（/api/embedding-providers）
 * - 多 provider（name / base_url / api_key / model / dim），单主用
 * - 主用切换后端 Invalidate 缓存 → 免重启热生效；影响知识录入 / 检索 / 反馈向量化
 * - dim=输出向量维度；当前 pgvector 列固定 1024，切换维度需离线迁移（后端已加 comment 提示）
 */
...
import type { EmbeddingProviderItem } from '@/types'

const loading = ref(false)
const items = ref<EmbeddingProviderItem[]>([])

async function load() {
  loading.value = true
  try {
    const resp = await http.get<EmbeddingProviderItem[]>('/embedding-providers')
...
const createForm = reactive({ name: '', base_url: '', api_key: '', model: '', dim: 1024 })

function openCreate() {
  Object.assign(createForm, { name: '', base_url: '', api_key: '', model: '', dim: 1024 })
...
async function submitCreate() {
  ...
  if (!Number.isInteger(createForm.dim) || createForm.dim <= 0) {
    ElMessage.warning('向量维度必须为正整数')
    return
  }
  createLoading.value = true
  try {
    await http.post('/embedding-providers', {
...
const editForm = reactive({
  id: 0, name: '', base_url: '', api_key: '', model: '', dim: 1024,
  api_key_placeholder: '',
})

function openEdit(row: EmbeddingProviderItem) {
...
async function submitEdit() {
  ...
  if (!Number.isInteger(editForm.dim) || editForm.dim <= 0) {
    ElMessage.warning('向量维度必须为正整数')
    return
  }
  editLoading.value = true
  try {
    await http.put(`/embedding-providers/${editForm.id}`, {
...
// ===== 设为主用 =====
async function activate(row: EmbeddingProviderItem) {
  try {
  ...
  try {
    await http.post(`/embedding-providers/${row.id}/activate`)
...
// ===== 删除 =====
async function remove(row: EmbeddingProviderItem) {
  const msg = row.is_active
    ? `「${row.name}」是当前主用。删除后若无其他主用，向量化将回落到 appsettings 配置。仍删除？`
    : `确定删除「${row.name}」？`
  try {
  ...
  try {
    await http.delete(`/embedding-providers/${row.id}`)
...
const testForm = reactive({ base_url: '', api_key: '', model: '', dim: 1024 })
const testResult = ref<{ ok: boolean; message?: string | null; got_dim?: number | null; latency_ms?: number } | null>(null)

function openTest(row: EmbeddingProviderItem) {
...
async function runTest() {
  ...
  testResult.value = null
  try {
    const resp = await http.post<{ ok: boolean; message?: string | null; got_dim?: number | null; latency_ms?: number }>(
      '/embedding-providers/test',
      {
...
<template>
  <div class="providers-tab">
    ...
    <el-table :data="items" v-loading="loading" size="small" stripe>
      ...
      <el-table-column label="维度" width="80" prop="dim" />
    ...
    <div class="empty-hint" v-if="items.length === 0">
      暂无 embedding 供应商。可点「新增供应商」添加，或先发起一次提问让系统自动带出默认配置。
    </div>

    <!-- 新增 -->
    <el-dialog v-model="createVisible" title="新增 embedding 供应商" width="560px">
      <el-form label-width="90px" label-position="left">
        <el-form-item label="名称" required>
          <el-input v-model="createForm.name" placeholder="如：本地 BGE / SiliconFlow Embedding" />
        </el-form-item>
        ...
        <el-form-item label="模型" required>
          <el-input v-model="createForm.model" placeholder="如 BAAI/bge-m3 / text-embedding-3-large" />
        </el-form-item>
        <el-form-item label="向量维度" required>
          <el-input-number v-model="createForm.dim" :min="1" :step="1" />
          <span class="form-hint">输出向量维度；当前 pgvector 列固定 1024，改维度需离线迁移</span>
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <el-dialog v-model="editVisible" :title="`编辑：${editForm.name}`" width="560px">
      <el-form label-width="90px" label-position="left">
        ...
        <el-form-item label="向量维度" required>
          <el-input-number v-model="editForm.dim" :min="1" :step="1" />
          <span class="form-hint">改维度后，新向量按新维度写入；存量旧维度数据需重嵌入</span>
        </el-form-item>
      </el-form>
      <template #footer>
    ...
    <!-- 测试 -->
    <el-dialog v-model="testVisible" title="测试 embedding 连通性" width="560px">
      <el-form label-width="90px" label-position="left">
        <el-form-item label="Base URL" required>
        ...
        <el-form-item label="期望维度" required>
          <el-input-number v-model="testForm.dim" :min="1" :step="1" />
          <span class="form-hint">实际返回维度需等于此值，测试才通过</span>
        </el-form-item>
      </el-form>
      <div v-if="testResult" class="test-result">
        ...
        >
          <template v-if="testResult.ok">
            实际返回维度 <strong>{{ testResult.got_dim }}</strong>
          </template>
          <template v-else>
            {{ testResult.message }}
          </template>
